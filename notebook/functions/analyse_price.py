...
def get_prices(ticker: str, start_date: str, end_date: str = None, interval: str = '1d', price_df_list=None) -> pd.DataFrame:
    """ 
...
def get_stats(returns_df_dict: str, ticker: str, start_period: str, end_year: str):
    ...
    stats_dict['n'] = period_of_interest_return_df[~period_of_interest_return_df[f"{mode}_Return (%)"].isna(
    )].shape[0]
    ...
    X = returns_df_dict['^AORD'].loc[regression_start_period:
                                     regression_end_period][f'{mode}_Return (%)']
    y = returns_df_dict[ticker].loc[regression_start_period:
                                    regression_end_period][f'{mode}_Return (%)']
...
def plot_returns(monthly_returns_df_dict: dict, quarterly_returns_df_dict: dict, yearly_returns_df_dict: dict, ticker: str, first_end_of_quarter: str, last_end_of_quarter: str, underlying_ticker=None, **kwargs):
...
def get_historical_prices(my_portfolio_tickers: list, index_tickers: str, historical_prices: list = None) -> dict:
    """
    ...
    with mp.Pool(mp.cpu_count()) as pool:
        ...
        results = pool.starmap(fetch_ticker_price, [(
...
def get_gics_industry_weighted_mean(return_df_dict: dict, TICKER: str, my_portfolio_tickers: list, same_gics_industry_weight_dict: dict, index_tickers: list, mode: str, comparable_tickers: list, **kwargs) -> pd.DataFrame:
    GICS_Industry_Weighted_Mean = dd(float)
    ...
    for date in return_df_dict[TICKER].index:
        ...
        for ticker, weight in adjusted_weight_dict.items():
            GICS_Industry_Weighted_Mean[date] += weight * \
                return_df_dict[ticker].loc[date].values[0]
    ...
    return_df_dict['GICS I.WMean' if 'industry' in comparable_tickers['type']
                   else 'GICS S.WMean'] = weighted_mean_df.sort_index()
...
def plot_returns_comparative(monthly_returns_df_dict: dict, quarterly_returns_df_dict: dict, yearly_returns_df_dict: dict,
                             TICKER: str, first_end_of_quarter: str, last_end_of_quarter: str, comparable_tickers: dict, **kwargs):
    ...
    industry_key = 'GICS I.WMean' if 'industry' in comparable_tickers[
        'type'] else 'GICS S.WMean'
    ...
    plot_bar_returns(axs[0], x_monthly - bar_width, interested_monthly_returns_df,
                     bar_width, f'{TICKER} Monthly Returns (%)', ticker_color, x_labels_monthly)
    plot_bar_returns(axs[0], x_monthly, industry_monthly_returns, bar_width,
                     'GICS Industry Weighted Mean (%)', industry_color, x_labels_monthly)
    plot_bar_returns(axs[0], x_monthly + bar_width, aord_monthly_returns,
                     bar_width, '^AORD (%)', aord_color, x_labels_monthly)
    ...
    common_quarterly_index = get_common_index(
        interested_quarterly_returns_df, industry_quarterly_returns, aord_quarterly_returns)
    ...
    plot_bar_returns(axs[1], x_quarterly - bar_width, interested_quarterly_returns_df,
                     bar_width, f'{TICKER} Quarterly Returns (%)', ticker_color, x_labels_quarterly)
    plot_bar_returns(axs[1], x_quarterly, industry_quarterly_returns, bar_width,
                     'GICS Industry Weighted Mean (%)', industry_color, x_labels_quarterly)
    plot_bar_returns(axs[1], x_quarterly + bar_width, aord_quarterly_returns,
                     bar_width, '^AORD (%)', aord_color, x_labels_quarterly)
    ...
    plot_bar_returns(axs[2], x_yearly - bar_width, interested_yearly_returns_df,
                     bar_width, f'{TICKER} Yearly Returns (%)', ticker_color, x_labels_yearly)
    plot_bar_returns(axs[2], x_yearly, industry_yearly_returns, bar_width,
                     'GICS Industry Weighted Mean (%)', industry_color, x_labels_yearly)
    plot_bar_returns(axs[2], x_yearly + bar_width, aord_yearly_returns,
                     bar_width, '^AORD (%)', aord_color, x_labels_yearly)
