...
from params import FUNDEMENTALS_RAW_COLUMNS, CASHFLOW_ROWS, BALANCE_SHEET_ROWS, FUNDEMENTAL_ROWS
...
def get_fundemental_cols_dfs(object: yf.Ticker, TICKER: str) -> pd.DataFrame:
    ...
    for fundementals_row in FUNDEMENTAL_ROWS:
        try:
...
def create_compound_key_features(
    ...
    try:
        stock_fundementals["Net Profit"] = (
            stock_fundementals['Total Revenue'] -
            stock_fundementals['Interest Expense'] -
            stock_fundementals['Tax Provision']
        )
        ...
        for column in ['Operating Expense', 'Other Operating Expenses', 'Selling And Marketing Expense', 'General And Administrative Expense', 'Other Non Interest Expense']:
            if column in stock_fundementals.columns:
                stock_fundementals["Net Profit"] -= stock_fundementals[column]
    ...
    try:
        stock_fundementals["Total Asset"] = (
            ...
            + stock_fundementals["Stockholders Equity"]
            + stock_fundementals["Total Non Current Liabilities Net Minority Interest"]
            + stock_fundementals["Other Non Current Liabilities"]
        )
    except KeyError as e:
