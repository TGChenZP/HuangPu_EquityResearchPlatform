...
def create_compound_key_features(
    ...
    try:
        ...
        for column in ['Operating Expense', 'Other Operating Expenses', 'Selling And Marketing Expense', 'General And Administrative Expense', 'Other Non Interest Expense']:
            if column in stock_fundamentals.columns:
                stock_fundamentals["Net Profit"] -= stock_fundamentals[column]

        for column in ['Other on Operating Income Expense', 'Other Income Expense', 'Other Non Operating Income Expenses', 'Gain On Sale Of Business', 'Gain On Sale Of Security']:
            if column in stock_fundamentals.columns:
                stock_fundamentals["Net Profit"] += stock_fundamentals[column]
    ...
    try:
        stock_fundamentals["Total Asset"] = (
            stock_fundamentals["Current Liabilities"]
            + stock_fundamentals["Stockholders Equity"]
            + stock_fundamentals["Total Non Current Liabilities Net Minority Interest"]
            + stock_fundamentals["Other Non Current Liabilities"]
        )
    except KeyError as e:
...
def get_key_statistics(stock_fundamentals: pd.DataFrame, ticker: str) -> pd.DataFrame:
    try:
        stock_fundamentals["Net Profit Margin"] = (
    ...
    try:
        stock_fundamentals["Net Income Margin"] = (
    ...
    try:
        stock_fundamentals["RoA"] = (
            stock_fundamentals["Net Profit"] /
            stock_fundamentals["Total Asset"].replace(0, np.nan)
        )
    except:
    ...
    try:
        stock_fundamentals["Interest Coverage Ratio"] = (
            ...
            / stock_fundamentals["Interest Paid Supplemental Data"].replace(0, np.nan)
        )
    except KeyError as e:
...
def get_raw_fundamentals_stats(comparable_ASX_tickers_dict: dict, first_end_of_quarter: str, historical_prices_dict: dict, COUNTRY: str):
    ...
    return raw_fundamentals_stats_dict, object_dict
